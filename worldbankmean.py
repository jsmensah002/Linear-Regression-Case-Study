...
#Replacing the null values in the GDP per capita column, the population density, and the unemployment rate using mean
mean_GDPpercapita = worldbank_data['GDP per capita (USD)'].mean()
mean_Popdensity = worldbank_data['Population density (people per sq. km of land area)'].mean()
mean_unempl = worldbank_data['Unemployment (% of total labor force) (modeled ILO estimate)'].mean()

#Replacing the null values with the calculated mean values
worldbank_data['GDP per capita (USD)'] = worldbank_data['GDP per capita (USD)'].fillna(mean_GDPpercapita)
worldbank_data['Population density (people per sq. km of land area)'] = worldbank_data['Population density (people per sq. km of land area)'].fillna(mean_Popdensity)
worldbank_data['Unemployment (% of total labor force) (modeled ILO estimate)'] = worldbank_data['Unemployment (% of total labor force) (modeled ILO estimate)'].fillna(mean_unempl)
print(worldbank_data)
...
#Prediction tests
#Question: Predict the unemployment rate of Spain in the year 2026 with a GDP per capita (USD) of 50000, and Population Density (people per sq. km of land area) of 500
country = 'Spain'
...
score_for_mean = reg.score(x,y)
print(score_for_mean)
